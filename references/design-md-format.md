...
- **Colors** accept any valid CSS color value; `"#1A1C1E"`-style hex is the recommended default. Wide-gamut brand colors (`oklch()`, `hsl()`, `color-mix()`), 8-digit hex (`"#RRGGBBAA"`), and named colors are all accepted (verified against the current CLI) — preserve the source's native format rather than lossy-converting to hex. Do **not** use arrays (`surface: [light, dark]`): the current CLI rejects them with `is not a valid color`, which is a **build-failing error** (see Validation), not a warning. For dark/alternate themes, add separate scalar tokens (e.g. `surface-dark`) and describe the mapping in the Colors prose.
